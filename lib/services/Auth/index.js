import Core from "../Core";// Importe la classe Core depuis le fichier ../Core


class Auth extends Core {// Définit une classe Auth qui hérite de la classe Core
    constructor(){
        super()// Appelle le constructeur de la classe parente
    }

    async loginUser(username, password) {// Définit une méthode asynchrone loginUser qui prend deux arguments: le nom d'utilisateur et le mot de passe
        try {
            
            if (!username || !password) {// Vérifie si l'un des deux paramètres est manquant
